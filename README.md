...
线上部署版本需要使用 Google 账号登录后进入应用。由于 OAuth 应用仍处于开发测试阶段，登录时 Google 会显示「此应用未经 Google 验证」安全提示；这是预期现象，点击左下角 **「高级」**，再点击 **「继续访问 coreshift-production.up.railway.app（不安全）」** 即可继续体验。
...
| 服务 | 说明 | 是否必须 |
| --- | --- | --- |
| **讯飞开放平台 IAT WebSocket** | 实时语音识别（STT） | 否（有文字输入兜底） |
| **讯飞开放平台 TTS WebSocket** | 流式语音播报（TTS） | 否（有浏览器 TTS 兜底） |
| **Web Speech API**（浏览器内置） | STT 降级兜底 | 需要 Chrome / Edge |
| **window.speechSynthesis**（浏览器内置） | TTS 降级兜底 / 讯飞不可用时使用 | — |
...
- **语音识别**：优先使用讯飞 IAT WebSocket 实时转写（需配置 `XUNFEI_APP_ID` / `XUNFEI_API_KEY` / `XUNFEI_API_SECRET`），浏览器端采集 16kHz PCM 音频并直连讯飞 WebSocket。讯飞不可用时自动降级为浏览器 Web Speech API，再不可用则降级为纯文字输入。
- **TTS 语音播报**：
  - **在线**：优先调用讯飞 TTS WebSocket 流式合成（需配置同一组 `XUNFEI_*` 环境变量），接收 16kHz PCM 后边收边播。
...
线上站点已启用 Google OAuth 登录。首次进入时请使用 Google 账号授权登录；应用尚未完成 Google 官方验证，因此会出现「此应用未经 Google 验证」提示，可按下方说明继续访问。

---

## Google 登录与日历授权

CoreShift 线上部署版本需要 Google 账号登录后进入应用，并支持将日程同步到 Google 日历。点击右上角「连接 Google 日历」按钮可开启日历同步授权流程。

### ⚠️ 授权时出现「此应用未经 Google 验证」警告

由于本项目处于开发测试阶段，尚未提交 Google OAuth 应用审核，登录或日历授权页面会显示如下警告：
...
- Node.js ≥ 18
- 现代浏览器（语音输入需要麦克风权限；讯飞不可用时 Chrome / Edge 可使用 Web Speech API 兜底）
...
# 4. （可选）配置环境变量，不配置功能可正常运行，语音会降级为浏览器能力或文字输入
# 在项目根目录创建 .env.local：
# XUNFEI_APP_ID=你的讯飞 AppID
# XUNFEI_API_KEY=你的讯飞 APIKey
# XUNFEI_API_SECRET=你的讯飞 APISecret
...
- 讯飞 ASR / TTS 需要网络连接和有效 `XUNFEI_*` 凭证；线上部署时必须在部署平台配置这些服务端环境变量，不能使用 `NEXT_PUBLIC_` 前缀。
- 中文时间解析支持常见口语表达，对极罕见的复合嵌套时间表达（如"下下下个月第三个周二"）可能解析失败，此时会提示追问。
- 讯飞不可用时会尝试降级为浏览器 Web Speech API / `speechSynthesis`；浏览器能力也不可用时，语音输入降级为文字输入。
...
核心创建、查询、修改、删除能力不依赖外部 API Key；讯飞语音服务、DeepSeek 和 Google Calendar 为可选增强功能。
...
│   ├── api/xunfei/         # 讯飞 ASR / TTS WebSocket 签名 URL
