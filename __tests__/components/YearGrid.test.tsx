...
// YearGrid renders prev/current/next year panels simultaneously.
// All queries must be scoped to the center panel via within().
function getPanel(year: number) {
  return within(screen.getByTestId(`year-panel-${year}`));
}

describe('YearGrid', () => {
  it('renders 12 month cards', () => {
    render(<YearGrid year={2026} events={[]} onMonthClick={jest.fn()} />);
    const panel = getPanel(2026);
    ['1月','2月','3月','4月','5月','6月','7月','8月','9月','10月','11月','12月']
      .forEach(m => expect(panel.getByText(m)).toBeInTheDocument());
  });

  it('shows event dot for dates with events', () => {
    render(<YearGrid year={2026} events={events} onMonthClick={jest.fn()} />);
    expect(getPanel(2026).getAllByTestId('event-dot').length).toBeGreaterThan(0);
  });

  it('does not show event dots when no events', () => {
    render(<YearGrid year={2026} events={[]} onMonthClick={jest.fn()} />);
    expect(getPanel(2026).queryAllByTestId('event-dot').length).toBe(0);
  });
  ...
  it('calls onMonthClick with correct month and year when card is clicked', () => {
    ...
    fireEvent.click(getPanel(2026).getByTestId('month-card-4')); // May = index 4
